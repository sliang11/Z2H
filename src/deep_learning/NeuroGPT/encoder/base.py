...
try:
    from torchinfo import ModelStatistics, summary  # type: ignore
except Exception:
    ModelStatistics = None  # type: ignore
...
class EEGModuleMixin():
    ...
    def get_torchinfo_statistics(
        ...
        # --- PATCH: fail only if someone calls this function ---
        if summary is None:
            raise ImportError(
                "torchinfo is not installed on this environment. "
                "Install it with `pip install --user torchinfo` or avoid calling get_torchinfo_statistics()."
            )
        # ------------------------------------------------------
        return summary(
            self,
    ...
    def __str__(self) -> str:
        # --- PATCH: don't crash just by printing the model ---
        if summary is None:
