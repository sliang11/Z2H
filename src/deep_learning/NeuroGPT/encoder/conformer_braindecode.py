...
from deep_learning.NeuroGPT.encoder.base import EEGModuleMixin, deprecated_args
...
class EEGConformer(EEGModuleMixin, nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        batch, chann, time = x.size()
        #exec(gen_cmd_print_variables('batch, chann, time', pfx_msg="********** "))

        x = x.contiguous().view(batch, chann, time)
        #exec(gen_cmd_print_variables('x.shape', pfx_msg='1. '))
        # x = x.permute(0, 2, 1, 3).contiguous().view(batch, chann, -1)

        x = torch.unsqueeze(x, dim=1)  # add one extra dimension
        #exec(gen_cmd_print_variables('x.shape', pfx_msg='2. '))
        x = self.patch_embedding(x)
        #exec(gen_cmd_print_variables('x.shape', pfx_msg='3. '))
        x = self.transformer(x)
        #exec(gen_cmd_print_variables('x.shape', pfx_msg='4. '))

        if self.is_decoding_mode:
            # pdb.set_trace()
            x = self.fc(x)
            #exec(gen_cmd_print_variables('x.shape', pfx_msg='5. '))
            x = self.final_layer(x)
            #exec(gen_cmd_print_variables('x.shape', pfx_msg='6. '))
        #exec(gen_cmd_print_variables('x.shape', pfx_msg='Final: '))
        return x
...
class _FullyConnected(nn.Module):
    def __init__(self, final_fc_length,
        ...
        print('final_fc_length', final_fc_length)
        self.linear_1 = nn.Linear(final_fc_length, out_channels)
        self.elu_1 = nn.ELU()
        self.dropout = nn.Dropout(drop_prob_1)
        self.linear_2 =nn.Linear(out_channels, hidden_channels)
        self.elu_2 = nn.ELU()
...
class _FinalLayer(nn.Module):
    def __init__(self, n_classes, hidden_channels=32, return_features=False, add_log_softmax=True):
        ...
        print("add_log_softmax", add_log_softmax)
        if add_log_softmax:
