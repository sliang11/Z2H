...
from deep_learning.NeuroGPT.batcher.adapter import EEGSpatialAdapter
import torch
import os
import argparse
# import pdb
...
import pandas as pd
import numpy as np
from deep_learning.NeuroGPT.encoder.conformer_braindecode import EEGConformer
...
PRETRAINED_MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pytorch_model.bin")
...
class SEEGArrayDataset(torch.utils.data.Dataset):

    def __init__(self, examples: np.ndarray, labels: np.ndarray, target_len: int = 500, normalization: bool = True):
        assert examples.ndim == 3, f"examples must be (N,C,T), got {examples.shape}"
        assert labels.ndim == 1, f"labels must be (N,), got {labels.shape}"
        assert len(examples) == len(labels), "examples/labels mismatch"
        self.samples = examples.astype(np.float32, copy=False)
    ...
    def _fix_len(self, x: np.ndarray) -> np.ndarray:
        ...
        out = np.zeros((c, self.target_len), dtype=x.dtype)
        out[:, :t] = x
        return out

    def __getitem__(self, idx):
        x = self.samples[idx]  # (C,T)
...
def prf_binary(y_true: np.ndarray, y_pred: np.ndarray, beta: float = 1.0):
    ...
    return float(precision), float(recall), float(f_beta)
...
def train(config: Dict = None) -> Trainer:
    ...
    for tvt_id in ('train', 'val', 'test'):
        p_ex, n_ex = load_seanet_format(tvt_id, config['base_seanet_save_path'], config['main_dl_data_path'])
        X = np.concatenate((p_ex, n_ex))
        y = np.concatenate((np.ones(len(p_ex)), np.zeros(len(n_ex))))
        all_X.append(X)
        all_y.append(y)
    x_tr, x_va, x_te = all_X
    y_tr, y_va, y_te = all_y

    config["input_channels"] = int(x_tr.shape[1])
    ...
    train_dataset = SEEGArrayDataset(
        examples=x_tr,
        labels=y_tr,
        target_len=config["chunk_len"],
        normalization=config.get("do_normalization", True),
    )
    validation_dataset = SEEGArrayDataset(
        examples=x_va,
        labels=y_va,
        target_len=config["chunk_len"],
        normalization=config.get("do_normalization", True),
    )
    test_dataset = SEEGArrayDataset(
        examples=x_te,
        labels=y_te,
        target_len=config["chunk_len"],
        normalization=config.get("do_normalization", True),
    ...
    assert trainer.args.gradient_accumulation_steps == 1
    ...
    if test_dataset is not None:
        ...
        if logits.ndim == 2:
            raw_pred = np.argmax(logits, axis=1).astype(np.int64)
        else:
            raw_pred = np.asarray(logits).reshape(-1).astype(np.int64)

        with open(os.path.join(config["log_dir"], "raw_predictions.pkl"), "wb") as f:
            pickle.dump(raw_pred.astype(np.int64), f)

        y_true = np.asarray(test_prediction.label_ids).astype(np.int64)
        if config.get("num_decoding_classes", 2) == 2:
            y_true_bin = (y_true > 0).astype(np.int64)
            y_pred_bin = (raw_pred > 0).astype(np.int64)
        else:
            y_true_bin = y_true
            y_pred_bin = raw_pred

        precision, recall, f1 = prf_binary(y_true_bin, y_pred_bin, beta=1.0)
        with open(os.path.join(config["log_dir"], "prediction_prf_with_beta=1.pkl"), "wb") as f:
            pickle.dump((precision, recall, f1), f)

        with open(os.path.join(config["log_dir"], "prediction_prf_with_beta=1.txt"), "w") as f:
...
def make_model(model_config: Dict = None):
    ...
    input_channels = int(model_config["input_channels"])  # no default: must be set from data
    target_channels = 22

    if input_channels == target_channels:
        adapter = nn.Identity()
    else:
        adapter = EEGSpatialAdapter(input_channels, target_channels)
...
def get_config(args: argparse.Namespace = None) -> Dict:
    ...
    config['main_dl_data_path'] = main_dl_data_path
    config['base_seanet_save_path'] = f'{args.data_id}_{channel_id}'
    config['log_dir'] = dl_result_path
...
def get_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help='nombre de canaux d’entrée des données'
    )
    ...
    parser.add_argument('--eval-every-n-steps', default=1000, type=int)
