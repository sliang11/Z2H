...
class ResNet(nn.Module):
    def __init__(self, conf: Conf):
        ...
        self.__pool = nn.Sequential(
            ...
            Squeeze(exclude_batch_dim=True)
        )
...
class Res1d18(nn.Module):
    def __init__(self, conf: Conf):
        ...
        self.__infer = nn.Sequential(
            # nn.Linear(feature_channels * 4, num_class),
    ...
    def forward(self, input: Tensor, auxiliary_input: Tensor = None, refine: bool = False, detach: bool = False) -> Tensor:
        # exec(gen_cmd_print_variables('input.shape'))
        # print('self.__feature(input).shape: ', self.__feature(input).shape)
        # print('self.__cnn(self.__feature(input)).shape: ', self.__cnn(self.__feature(input)).shape)
        # print('self.__infer(self.__pool(self.__cnn(self.__feature(input)))).shape: ', self.__infer(self.__pool(self.__cnn(self.__feature(input)))).shape)
