...
class SelfTrainingTree(object):
    ...
    def get_chains_with_n(self, labels_by_ind):
        ...
        for node in self.nodes[1:]: # 跳过root
            ind = node.ind
            ...
            if ori_pos_first_n_in_chain != -1:  # 原先chain中就有负样本
                pos_first_n_in_chain = ori_pos_first_n_in_chain
            elif ori_pos_first_n_in_chain == -1 and labels_by_ind[ind] == -1:   # 原先chain中没有负样本，当前也不是负样本
                pos_first_n_in_chain = -1
            else:
                pos_first_n_in_chain = len(ori_chain)  # 当前ind是第一个负样本
